 ...
 */
package de.openknowledge.sample.onlineshop.infrastructure.common;
    ...
    protected Object instantiate(Class<?> type, Function<String, Object> value, Set<ConstraintViolation<?>> violations) {
        if (type.isAnnotationPresent(JsonbSubclass.class) || type.isAnnotationPresent(JsonbSubclass.List.class)) {
            ...
            if (discriminator != null) {
                if (value.apply(discriminator.discriminatorProperty()) != null) {
            ...
            if (list != null) {
                for (JsonbSubclass s: list.value()) {
                    if (value.apply(s.discriminatorProperty()) != null) {
                        type = s.subclass();
                    }
                }
            }
