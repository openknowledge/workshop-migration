 ...
 */
package de.openknowledge.sample.order.infrastructure.common;
...
import de.openknowledge.sample.order.infrastructure.jsonb.JsonbSubclass;
...
public abstract class AbstractRecordConverter {
    ...
    protected Object instantiate(Class<?> type, Map<String, Object> value) {
        if (type.isAnnotationPresent(JsonbSubclass.class) || type.isAnnotationPresent(JsonbSubclass.List.class)) {
            JsonbSubclass discriminator = type.getAnnotation(JsonbSubclass.class);
            if (discriminator != null) {
                if (value.containsKey(discriminator.discriminatorProperty())) {
                    type = discriminator.subclass();
                }
            }
            JsonbSubclass.List list = type.getAnnotation(JsonbSubclass.List.class);
            if (list != null) {
                for (JsonbSubclass s: list.value()) {
                    if (value.containsKey(s.discriminatorProperty())) {
                        type = s.subclass();
                    }
                }
            }
