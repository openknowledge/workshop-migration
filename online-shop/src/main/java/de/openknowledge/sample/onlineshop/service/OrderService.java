...
public class OrderService {
    ...
    public OrderAggregate initializeShoppingCart(CustomerAggregate customer) {
        try {
            return repository.findByCustomerAndStatus(customer.getCustomerNumber(), OrderStatus.SHOPPING_CART);
        } catch (NoResultException e) {
            OrderAggregate shoppingCart = new OrderAggregate(customer, OrderStatus.SHOPPING_CART);
            repository.persist(shoppingCart);
            return shoppingCart;
        }
    }
    ...
    public OrderNumber createOffer(ShoppingCart shoppingCart) {
        OrderAggregate order = repository.findByCustomerAndStatus(shoppingCart.customerNumber(), OrderStatus.SHOPPING_CART);
    ...
    public OrderNumber order(OrderSummary summary) {
        OrderAggregate order = repository.findByCustomerAndStatus(summary.customerNumber(), OrderStatus.OFFERED);
