...
public class OfferResource  {
    ...
    public Response createOrder(@PathParam("offerNumber") @Valid OfferNumber number) {
        ...
        Response orderResponse = ClientBuilder.newClient().target(onlineShopUrl).path("online-shop/orders").request().post(json(order));
        ...
        } else if (orderResponse.getStatus() == NOT_FOUND.getStatusCode()) {
            return Response.seeOther(externalUrl.resolve(format("/offers/%s/not-found", number.number()))).build();
        } else {
    ...
    @GET
    @Path("/{offerNumber}/not-found")
    @Produces({ "text/html" })
    public String showNotFoundPage(@PathParam("offerNumber") @Valid OfferNumber offerNumber) {
        offerBean.setNumber(offerNumber);
        return templateProcessor.apply("not-found");
    }
